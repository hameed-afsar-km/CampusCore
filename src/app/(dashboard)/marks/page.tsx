...
export default function MarksTrackerPage() {
  const { data: semestersRaw, add, update, remove, loading } = useFirestore<Semester>("marks");
  ...
  const [confirmDelete, setConfirmDelete] = useState<{ id: string, type: "subject" | "semester" } | null>(null);

  // Sorting semesters by a timestamp or some counter is better, here we use the order in array
  const semesters = useMemo(() => {
    return [...semestersRaw].sort((a, b) => a.label.localeCompare(b.label));
  }, [semestersRaw]);

  // Handle initial semester if none exists
  useEffect(() => {
    if (!loading && semestersRaw.length === 0) {
      add({ label: "Semester 1", gpa: null, subjects: [] });
    }
  }, [loading, semestersRaw.length, add]);

  const currentSem = semesters[currentSemIndex] || semesters[0];
  ...
  const bestSubject = subjectsWithCalc.reduce<any>(
    (prev, cur) => (cur.finalPercent !== null && (prev === null || cur.finalPercent > (prev.finalPercent ?? 0))) ? cur : prev,
    null
  );
  const worstSubject = subjectsWithCalc.reduce<any>(
    (prev, cur) => (cur.finalPercent !== null && (prev === null || cur.finalPercent < (prev.finalPercent ?? Infinity))) ? cur : prev,
    null
  );

  // Handlers
  const updateSubjectMark = async (subId: string, field: keyof Subject, value: number | null) => {
    if (!currentSem) return;
    const newSubjects = currentSem.subjects.map((s) => (s.id === subId ? { ...s, [field]: value } : s));
    await update(currentSem.id, { subjects: newSubjects });
  };

  const handleSaveSubject = async (formData: Omit<Subject, OmitData>) => {
    if (!currentSem) return;
    let newSubjects = [...currentSem.subjects];
  ...
  const deleteSubject = async () => {
    if (!confirmDelete || !currentSem) return;
    const newSubjects = currentSem.subjects.filter((s) => s.id !== confirmDelete.id);
    await update(currentSem.id, { subjects: newSubjects });
  ...
  const deleteSemester = async () => {
    ...
    setConfirmDelete(null);
    setCurrentSemIndex(Math.max(0, currentSemIndex - 1));
  };

  const handlePromptDelete = (id: string, type: "subject" | "semester") => {
  ...
  return (
    <div className="space-y-6">
          ...
          {semesters.map((sem, idx) => (
            <div key={sem.id} className="relative group flex items-center">
              ...
              {idx === currentSemIndex && semesters.length > 1 && (
                <button
                  onClick={(e) => { e.stopPropagation(); handlePromptDelete(sem.id, "semester"); }}
      ...
      {loading ? (
        <div className="flex items-center justify-center py-20">
          <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1, ease: "linear" }}>
            <BarChart2 className="w-10 h-10 text-purple-500/50" />
          </motion.div>
        </div>
      ) : (
        <>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  onClick={async () => {
                    ...
                    if (isNaN(gpa) || gpa < 0 || gpa > 10 || !currentSem) return;
                    await update(currentSem.id, { gpa });
                    setPrevGpaInput("");
                  }}
                      ...
                      <tr key={sub.id} className="hover:bg-white/[0.02] transition-colors group">
                        ...
                        <td className="py-3 px-3 text-right">
                          <div className="flex items-center justify-end gap-1 opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity">
                            ...
                            <button onClick={() => handlePromptDelete(sub.id, "subject")} className="p-1.5 rounded-lg text-red-400 hover:text-white hover:bg-red-500/20 transition-all">
      ...
      <ConfirmModal
        ...
        onConfirm={confirmDelete?.type === "subject" ? deleteSubject : deleteSemester}
        title={confirmDelete?.type === "subject" ? "Delete Subject?" : "Delete Semester?"}
        message="This action cannot be undone and will remove all associated data."
      />
    </div>
  );
}
