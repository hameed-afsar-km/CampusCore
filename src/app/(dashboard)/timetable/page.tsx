...
interface Subject {
  ...
  code: string;
  name: string;
  alias?: string;       // Short name like CCN, OS, etc.
  credits: number;
  facultyId: string;
  facultyName: string;
}
              ...
              {professors
                ...
                .map((p: any) => (
                  ...
                  >
                    ...
                    {p.displayName}
                    {p.staffId && <span className="text-[10px] opacity-60">({p.staffId})</span>}
                  </button>
                ))}
              {professors.filter((p: any) => !facultySearch || p.displayName?.toLowerCase().includes(facultySearch.toLowerCase())).length === 0 && (
                <p className="text-xs text-gray-500 italic">No faculty matching "{facultySearch}"</p>
              )}
            </div>
          )}
        </div>
                    ...
                    return (
                      <tr key={day} className={isToday ? "bg-cyan-500/[0.03] relative group/today" : ""}>
                        ...
                        {TIME_SLOTS.map((slot, index) => {
                          ...
                          return (
                            <td key={slot.id} colSpan={mergeColSpan} className="p-1 border-r border-white/[0.06] relative group align-top min-h-[110px]">
              ...
              <form onSubmit={handleSaveSubject} className="space-y-3">
                 ...
                 <div className="grid grid-cols-3 gap-4">
                  <div className="col-span-2">
                    <label className="text-xs font-bold text-gray-500 uppercase block mb-2 tracking-wider">Subject Name *</label>
                    <input required value={subjectForm.name} onChange={(e) => setSubjectForm({ ...subjectForm, name: e.target.value })} placeholder="e.g. Operating Systems" className="w-full bg-white/[0.03] border border-white/[0.08] focus:border-purple-500/50 rounded-xl py-2.5 px-4 text-sm text-gray-200 outline-none transition-all" />
                  </div>
                  <div>
                    <label className="text-xs font-bold text-gray-500 uppercase block mb-2 tracking-wider">Alias</label>
                    <input value={subjectForm.alias || ""} onChange={(e) => setSubjectForm({ ...subjectForm, alias: e.target.value })} placeholder="e.g. OS" className="w-full bg-white/[0.03] border border-white/[0.08] focus:border-purple-500/50 rounded-xl py-2.5 px-4 text-sm text-gray-200 outline-none transition-all" />
                  </div>
                </div>
                 ...
                 <select value={subjectForm.facultyId} onChange={e => setSubjectForm({ ...subjectForm, facultyId: e.target.value })} className="w-full bg-white/[0.03] border border-white/[0.08] focus:border-purple-500 rounded-lg p-2.5 text-sm outline-none transition-colors">
                    <option value="" className="bg-[#0a0e17]">Assign Faculty (Optional for Library/CA)</option>
